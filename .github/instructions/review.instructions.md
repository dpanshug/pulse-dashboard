...
- **Module Server Routes:** Any new or modified route files within `modules/*/server/` that implement non-trivial business logic, data aggregation, or stateful operations.
...
2. **Flag Non-Compliance:** If any files within `modules/` were changed but **no matching integration test** is detected in the diff:
   ...
   - Explicitly highlight which module files were modified without verification (e.g., *"You modified data fetching logic inside `modules/upstream-pulse/server/index.js` without corresponding integration assertions"* or *"You added a new view in `modules/upstream-pulse/views/` without integration test coverage"*).
